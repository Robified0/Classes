...
execute unless entity @s[tag=cl.s.Poison] run function classes:mage/spells/all/noknowledge

#Copy current slot # to spell scoreboard, only if player knows spell.
execute if entity @s[tag=cl.s.Poison] run scoreboard players operation @s cl.s.Poison = @s cl.loadSpell
...
execute if entity @s[tag=cl.s.Poison] as @s run function classes:operations/increasescore
