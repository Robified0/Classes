...
execute as @s[scores={cl.Mana=3..}] run scoreboard players set @s cl.e.IronFlesh 0
execute as @s[scores={cl.Mana=3..}] run tag @s remove StoneFlesh
execute as @s[scores={cl.Mana=3..}] run tag @s remove IronFlesh

#Give Player Tag for particle effect
execute as @s[scores={cl.Mana=3..}] run tag @s add DiaFlesh
execute as @s[scores={cl.Mana=3..}] run scoreboard players set @s cl.e.DiaFlesh 3600

#Change base attribute
execute as @s[scores={cl.Mana=3..}] run attribute @s minecraft:generic.armor base set 12

#Particle effect
execute as @s[scores={cl.Mana=3..}] at @s run particle block diamond_block ~ ~ ~ 1 1 1 10 40 normal
...
execute as @s[scores={cl.Mana=3..}] run tellraw @s[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" casted","color":"green"},{"text":" Diamond Flesh!","bold":true,"color":"#AAAAAA"}]
