...
execute if entity @s[scores={cl.Mana=3..}] run scoreboard players set @s cl.e.StoneFlesh 0
execute if entity @s[scores={cl.Mana=3..}] run scoreboard players set @s cl.e.IronFlesh 0
execute if entity @s[scores={cl.Mana=3..}] run tag @s remove StoneFlesh
...
execute if entity @s[scores={cl.Mana=3..}] at @s run playsound minecraft:entity.zombie_villager.cure player @a ~ ~ ~ 0.5 1.5

#Cast Spell Sound Effect
execute if entity @s[scores={cl.Mana=3..}] at @s run function classes:mage/spells/all/cast

#Spell Cast Notification
execute if entity @s[scores={cl.Mana=3..}] run tellraw @s[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" casted","color":"green"},{"text":" Diamond Flesh!","bold":true,"color":"#AAAAAA"}]
