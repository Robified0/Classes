...
execute if entity @s[scores={Mana=1..}] run summon minecraft:marker ~ ~ ~ {Age:-2147483648,Duration:-1,WaitTime:-2147483648,Tags:["new","slowcast", "fireball"],Attributes:[{AttributeName:"generic.attackKnockback",Name:"generic.attackKnockback",Base:2,Operation:0,UUIDMost:0,UUIDLeast:10}]}
execute if entity @s[scores={Mana=1..}] as @e[type=marker,tag=slowcast,tag=new,tag=fireball,limit=1] run function classes:mage/spells/fireball/zprivate/setup
...
execute if entity @s[scores={Mana=1..}] run tellraw @a[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" casted","color":"green"},{"text":" Fireball!","bold":true,"color":"#FF3034"}]
