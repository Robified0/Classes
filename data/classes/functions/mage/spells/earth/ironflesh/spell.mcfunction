...
execute if entity @s[scores={cl.Mana=8..}] run tag @s add IronFlesh
execute if entity @s[scores={cl.Mana=8..}] run scoreboard players set @s cl.e.IronFlesh 24000

#Change base attribute
execute if entity @s[scores={cl.Mana=8..}] run attribute @s minecraft:generic.armor base set 12

#Particle effect
execute if entity @s[scores={cl.Mana=8..}] at @s run particle block iron_block ~ ~ ~ 1 1 1 10 40 normal
...
execute if entity @s[scores={cl.Mana=8..}] run tellraw @s[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" cast","color":"green"},{"text":" Iron Flesh!","bold":true,"color":"#AAAAAA"}]
