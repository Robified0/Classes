...
execute as @e[type=marker,tag=cl.r.slowcast,tag=cl.r.new,tag=cl.r.poison,limit=1] run function classes:mage/spells/earth/poison/zprivate/setup
...
tellraw @a[tag=cl.p.SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" cast","color":"green"},{"text":" Poison!","bold":true,"color":"#03800F"}]
