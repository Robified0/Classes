...
execute unless entity @s[tag=cl.s.IceShard] run function classes:mage/spells/all/noknowledge

#Copy current slot # to spell scoreboard, only if player knows spell.
execute if entity @s[tag=cl.s.IceShard] run scoreboard players operation @s cl.s.IceShard = @s cl.loadSpell
...
execute if entity @s[tag=cl.s.IceShard] as @s run function classes:operations/increasescore
