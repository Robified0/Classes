...
execute if entity @s[scores={cl.Mana=2..}] run summon minecraft:marker ~ ~ ~ {Age:-2147483648,Duration:-1,WaitTime:-2147483648,Tags:["new","slowcast", "iceshard"]}
execute if entity @s[scores={cl.Mana=2..}] as @e[type=marker,tag=slowcast,tag=new,tag=iceshard,limit=1] run function classes:mage/spells/iceshard/zprivate/setup
...
execute if entity @s[scores={cl.Mana=2..}] at @s run particle dust 0.129 0.914 1.000 1 ~ ~ ~ 0.5 1 0.5 1 50 force

#Cooldown
execute as @s[scores={cl.Mana=2..}] run scoreboard players set @s cl.Cooldown 20

#Spell Cast Notification
execute if entity @s[scores={cl.Mana=2..}] run tellraw @a[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" casted","color":"green"},{"text":" Ice Shard!","bold":true,"color":"#0096FF"}]
