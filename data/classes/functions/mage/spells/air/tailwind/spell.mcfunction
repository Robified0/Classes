...
execute if entity @s[scores={cl.Mana=4..}] run tag @s add TailWind

execute if entity @s[scores={cl.Mana=4..}] run scoreboard players set @s cl.e.TailWind 400

#Give Effect
execute if entity @s[scores={cl.Mana=4..}] run effect give @s minecraft:speed 20 1 false

#Particle effect
execute if entity @s[scores={cl.Mana=4..}] at @s run particle minecraft:cloud ^0.5 ^ ^-0.5 1 1 1 10 40 normal
...
execute if entity @s[scores={cl.Mana=4..}] run tellraw @s[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" cast","color":"green"},{"text":" Tail Wind!","bold":true,"color":"#FFFF80"}]
