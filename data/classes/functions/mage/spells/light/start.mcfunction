...
execute if entity @s[scores={Mana=1..}] run summon minecraft:marker ~ ~ ~ {Age:-2147483648,Duration:-1,WaitTime:-2147483648,Tags:["new","slowcast", "light"]}
execute if entity @s[scores={Mana=1..}] as @e[type=marker,tag=slowcast,tag=new,tag=light,limit=1] run function classes:mage/spells/light/zprivate/setup
...
execute if entity @s[scores={Mana=1..}] at @s run particle minecraft:end_rod ~ ~0.5 ~ 0.3 1 0.3 0.03 50 force
...
execute if entity @s[scores={Mana=1..}] run tellraw @a[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" casted","color":"green"},{"text":" Light!","bold":true,"color":"#FFFFFF"}]
