...
#Give Effect
execute as @s[scores={cl.Mana=4..}] run tag @s add ColdShield

#Set time limit for spell Effect
execute as @s[tag=ColdShield] run scoreboard players set @s cl.e.ColdShield 400

#Give Effect
execute as @s[scores={cl.Mana=4..}] run effect give @s minecraft:fire_resistance 20 0 true

#Particle effect
execute as @a[scores={cl.Mana=4..}] at @s run particle dust 0.129 0.914 1.000 1 ~ ~ ~ 1.5 1.5 1.5 1 100 normal

#Playsound Effect
execute at @s[scores={cl.Mana=4..}] run playsound minecraft:block.fire.extinguish player @a ~ ~ ~ 0.8 2
