...
execute unless entity @s[tag=cl.s.Teleport] run function classes:mage/spells/all/noknowledge

#Copy current slot # to spell scoreboard, only if player knows spell.
execute if entity @s[tag=cl.s.Teleport] run scoreboard players operation @s cl.s.Teleport = @s cl.loadSpell
...
execute if entity @s[tag=cl.s.Teleport] as @s run function classes:operations/increasescore
