...
execute unless block ~ ~ ~ air positioned ^ ^ ^-1.5 as @e[type=marker,tag=teleport,sort=nearest,limit=1] if score @s cl.s.Teleport.t = @p[tag=Teleport,sort=nearest,limit=1] cl.s.Teleport.t run tp @p[tag=Teleport,sort=nearest,limit=1] @s
