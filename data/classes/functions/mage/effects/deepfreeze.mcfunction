...
execute as @e[tag=DeepFreeze,scores={cl.e.DeepFreeze=1..}] at @s positioned ~ ~1 ~ run kill @e[type=item,nbt={Item:{id:"minecraft:ice",Count:1b}},distance=..1.5]
execute if entity @e[scores={cl.e.DeepFreeze=1..}] run scoreboard players remove @e[scores={cl.e.DeepFreeze=1..}] cl.e.DeepFreeze 1
execute as @e[tag=DeepFreeze,scores={cl.e.DeepFreeze=..0}] at @s run fill ~-1 ~-1 ~-1 ~2 ~2 ~2 air replace ice
