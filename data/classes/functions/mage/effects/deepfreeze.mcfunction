execute as @e[tag=DeepFreeze,scores={cl.e.DeepFreeze=1..}] at @s unless block ~ ~ ~ packed_ice unless block ~ ~1 ~ packed_ice run scoreboard players set @s cl.e.DeepFreeze 0
execute as @e[tag=DeepFreeze,scores={cl.e.DeepFreeze=1..}] at @s run particle dust 0.129 0.914 1.000 1 ~ ~ ~ 0.4 0.4 0.4 2 10 force
execute as @e[tag=DeepFreeze,scores={cl.e.DeepFreeze=1..}] run data merge entity @s {Fire:-20}
