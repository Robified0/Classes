...
execute if entity @s[scores={cl.Mana=2..}] run summon minecraft:marker ~ ~ ~ {Age:-2147483648,Duration:-1,WaitTime:-2147483648,Tags:["new","slowcast", "heal"]}
execute if entity @s[scores={cl.Mana=2..}] as @e[type=marker,tag=slowcast,tag=new,tag=heal,limit=1] run function classes:healer/spells/heal/zprivate/setup
...
#Particles
execute if entity @s[scores={cl.Mana=2..}] at @s run particle minecraft:wax_off ~ ~ ~ 0.5 1 0.5 0.01 10 force
...
execute if entity @s[scores={cl.Mana=2..}] run tellraw @a[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" casted","color":"green"},{"text":" Heal!","bold":true,"color":"green"}]
