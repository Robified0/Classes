...
execute unless entity @s[x_rotation=87..90] if entity @s[scores={cl.Mana=1..}] run scoreboard players set dst_per cl.slowcast 4
# how many half blocks to travel (24*0.5) = 12 blocks
execute unless entity @s[x_rotation=87..90] if entity @s[scores={cl.Mana=1..}] run scoreboard players set dst cl.slowcast 48
#Start raycast if caster isn't looking down.
execute unless entity @s[x_rotation=87..90] if entity @s[scores={cl.Mana=1..}] at @s positioned ~ ~1.45 ~ run function classes:healer/spells/heal/start
...
execute if entity @s[scores={cl.Mana=1..}] run tellraw @a[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" cast","color":"green"},{"text":" Heal!","bold":true,"color":"green"}]

#Mana Depletion
execute if entity @s[scores={cl.Mana=1..},x_rotation=87..90] run scoreboard players remove @s cl.Mana 1

#Modify item (for spell versioning)
item modify entity @s weapon.mainhand classes:healer/spells/heal
