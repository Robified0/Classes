...
execute unless entity @s[x_rotation=87..90] if entity @s[scores={cl.Mana=2..}] at @s positioned ~ ~1.45 ~ run function classes:healer/spells/healing/satiate/start
...
execute if entity @s[scores={cl.Mana=2..}] run tellraw @a[tag=SpellNotify,distance=..40] ["",{"selector":"@s"},{"text":" cast","color":"green"},{"text":" Satiate!","bold":true,"color":"green"}]
