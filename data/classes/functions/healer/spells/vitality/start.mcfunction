...
execute if entity @s[scores={cl.Mana=4..}] run scoreboard players operation @s cl.slo_id = system cl.slo_id
execute if entity @s[scores={cl.Mana=4..}] run summon minecraft:marker ~ ~ ~ {Age:-2147483648,Duration:-1,WaitTime:-2147483648,Tags:["new","slowcast", "vitality"]}
execute if entity @s[scores={cl.Mana=4..}] as @e[type=marker,tag=slowcast,tag=new,tag=vitality,limit=1] run function classes:healer/spells/vitality/zprivate/setup
execute if entity @s[scores={cl.Mana=4..}] run tag @s remove this
