...
execute at @s[scores={cl.Class=2}] as @e[type=#classes:ranger/canown,tag=cl.t.Owned,tag=cl.t.Rangers,distance=..10] unless score @s cl.t.RangerID matches 1.. run function classes:ranger/system/taming/apply/tame

#Revoke advancement to allow re-triggering
execute if entity @s[advancements={classes:tameanimal=true}] run advancement revoke @s only classes:tameanimal
